...
window, with source page and draft position but no indexed identity. Produced from
[[LayoutBlock]]s by structure-aware chunking: a heading is kept with its following body, a table is
kept intact, blocks on different pages are never merged (so [[Citation]]s stay page-accurate), and
oversized content is split to a token budget with overlap.

### Chunk
An indexed [[DraftChunk]] whose stable `chunk_id` is assigned during ingestion from
`(document name, draft position)`. The unit that gets embedded, stored in metadata, and indexed.
